...
def _decrypt_database(db_path: str, out_path: str, enc_key: bytes) -> bool:
    ...
    # Decrypt WAL file if it exists (contains newest uncommitted data)
    wal_path = db_path + "-wal"
    if os.path.exists(wal_path) and os.path.getsize(wal_path) > 32:
        _decrypt_wal(wal_path, out_path, enc_key)
        # Checkpoint: open the DB in normal mode to replay WAL into main file
        # This makes the DB self-contained (no WAL dependency) for immutable reads
        try:
            import sqlite3
            conn = sqlite3.connect(out_path)
            conn.execute("PRAGMA wal_checkpoint(TRUNCATE)")
            conn.close()
            _print("  WAL: checkpoint complete")
        except Exception as e:
            _print(f"  WAL: checkpoint skipped ({e})")

    return True


def _decrypt_wal(wal_path: str, out_path: str, enc_key: bytes) -> None:
    """Decrypt WAL file and create a decrypted WAL alongside the output DB.

    SQLCipher WAL structure:
    - 32 bytes: WAL header (plaintext, copy as-is)
    - Repeated frames:
      - 24 bytes: frame header (plaintext, copy as-is)
      - PAGE_SZ bytes: encrypted page data → decrypt and write

    SQLite will automatically replay the decrypted WAL when the DB is opened,
    giving us the most up-to-date data including recent uncommitted writes.
    """
    wal_size = os.path.getsize(wal_path)
    frame_size = 24 + PAGE_SZ
    num_frames = (wal_size - 32) // frame_size

    if num_frames <= 0:
        return

    _print(f"  WAL: {num_frames} frames, decrypting...")

    out_wal_path = out_path + "-wal"
    with open(wal_path, "rb") as wal_f, open(out_wal_path, "wb") as out_wal_f:
        # Copy WAL header as-is (plaintext)
        wal_header = wal_f.read(32)
        out_wal_f.write(wal_header)

        applied = 0
        failed = 0
        for i in range(num_frames):
            # Read and copy frame header (24 bytes, plaintext)
            frame_header = wal_f.read(24)
            if len(frame_header) < 24:
                break
            page_number = struct.unpack(">I", frame_header[0:4])[0]
            out_wal_f.write(frame_header)

            # Read encrypted page data
            page_data = wal_f.read(PAGE_SZ)
            if len(page_data) < PAGE_SZ:
                break

            # Decrypt the page and write to output WAL
            try:
                decrypted_page = _decrypt_page(enc_key, page_data, page_number)
                out_wal_f.write(decrypted_page)
                applied += 1
            except Exception:
                # Write zeros if decrypt fails (shouldn't happen)
                out_wal_f.write(b"\x00" * PAGE_SZ)
                failed += 1

    _print(f"  WAL: decrypted {applied} frames" + (f" ({failed} failed)" if failed else ""))
